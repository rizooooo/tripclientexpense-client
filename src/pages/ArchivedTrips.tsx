...
const ArchivedTrips = () => {
  ...
  const queryArchivedTrips = useQuery({
    queryFn: async () => {
      const response = await tripApi.apiTripsArchivedGet();
  ...
  const mutationUnarchive = useMutation<void, Error, number>({
    onSuccess: () => {
      ...
      <div className="min-h-screen bg-gray-50 pb-20">
        ...
        <div className="bg-gradient-to-r from-gray-600 to-gray-700 text-white p-6 pb-8">
          ...
          <p className="text-gray-100 text-sm ml-9">
            ...
            {archivedTrips.length === 1 ? "trip" : "trips"} archived
          </p>
        </div>
          ...
          {archivedTrips.length === 0 ? (
            ...
            <div className="space-y-3">
              {archivedTrips.map((trip) => (
                ...
                >
                  {/* Combined Currency and Archive Badges - Top Right */}
                  ...
                  >
                    {/* Trip Name - pr-20 is removed since badges are now outside */}
                    <div className="flex items-start justify-between mb-2">
                      <h3 className="font-semibold text-gray-800">
                        {trip.name}
                      </h3>
                      {/* The currency badge was removed from here */}
                    </div>
